...
public:
  ...
  static line2 line_y(T p_x = T{0}) { return line2{point_type{p_x, 0}, vec_type{0, 1}}; }

  T signed_distance(const point_type &p_point) const { return dot(p_point - point_type::origin(), m_normal) - m_dist; }
  T distance(const point_type &p_point) const { return std::abs(signed_distance(p_point)); }
  T distance_origin() const { return std::abs(m_dist); }

  vec_type normal() const { return m_normal; }
};

template <typename T> T signed_distance_from_line2(const line2<T> &p_line, const point2<T> &p_point) {
  return p_line.signed_distance(p_point);
}

template <typename T> T distance_from_line2(const line2<T> &p_line, const point2<T> &p_point) {
